<template>
  ...
  <div class="bg-white">
    <div v-if="validarTabla" class="flex justify-center pt-4">
      ...
      <div class="mb-3 xl:w-96 px-2">
        ...
        >
        <select
          class="form-select appearance-none block w-full px-5 py-1.5 text-base font-normal text-gray-700 bg-white bg-clip-padding bg-no-repeat border border-solid border-gray-300 rounded transition ease-in-out m-0 focus:text-gray-700 focus:bg-white focus:border-green-900 focus:outline-none"
          v-model="ordenBusqueda"
        >
    ...
    <div v-if="validarTabla" class="flex justify-center pt-4">
      <button
        class="bg-black px-3 py-2 text-white font-bold text-sm rounded-md"
    ...
    >
      Los datos de los jugadores se estan cargando
    </div>
    <div
...
export default {
  ...
  methods: {
    ...
    async listarJugadores() {
      this.isLoading = true;
      var response;
      await ApiJugadores.TraerJugadores(this.datosApi)
        .then((respuesta) => {
          response = respuesta;
